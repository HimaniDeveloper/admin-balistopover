...
import { addHolidayData } from "@/store/holidaySlice";
...
export default function HolidayForm() {
  ...
  const handleChange = (e) => {
    ...
    let updatedHolidayData = { ...holidayData, [name]: value };
    if (name === "title") {
      const generatedRoutPath = value.toLowerCase().replace(/\s+/g, "_");
      updatedHolidayData = {
        ...updatedHolidayData,
        routPath: generatedRoutPath,
      };
    }
  ...
  const handleAddHoliday = async () => {
    setLoading(true);
    try {
      await dispatch(addHolidayData(holidayData)).unwrap();
      dispatch(
        showToast({
          type: "success",
      ...
      dispatch(showToast({ type: "error", message: "Failed to add holiday" }));
    } finally {
  ...
  return (
    <Layout title={"Add New Holiday Package"}>
      <Box p={3} mx="auto">
        <Box mt={2}>
          <HInput
            label="Title"
