...
import { Box, Typography, IconButton, Grid, Item } from "@mui/material";
import { useDispatch } from "react-redux";
import { showToast } from "@/store/settingsSlice";
import { PhotoCamera } from "@mui/icons-material";
import { HButton, HInput, Layout } from "@/components";
import { addPlaceData } from "@/store/popularplaceSlice";
...
const DestinationForm = ({ params }) => {
  ...
  const handleAddDestination = async () => {
    setLoading(true);
    try {
      await dispatch(addPlaceData(destinationData)).unwrap();
      dispatch(
        showToast({
          type: "success",
